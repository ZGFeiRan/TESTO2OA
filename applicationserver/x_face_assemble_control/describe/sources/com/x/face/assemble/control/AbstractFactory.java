...
public abstract class AbstractFactory {
	...
	@SuppressWarnings("unchecked")
	protected <T extends JpaObject> List<T> pick(List<String> FLAGS, Class<T> clz, String[] attributes)
			throws Exception {
		...
		if (map.size() == FLAGS.size()) {
			...
			List<T> os = this.entityManagerContainer().flag(FLAGS,clz,attributes);
			EntityManager em = this.entityManagerContainer().get(clz);
			os.stream().forEach(o -> {
				em.detach(o);
				list.add(o);
